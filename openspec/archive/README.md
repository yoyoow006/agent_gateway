...
- `add-agw-reload-command` `agw reload` 手动热重载 CLI：不写盘触发 `POST /__agw/reload`，网关未运行时幂等退出；退出码区分解析失败（1）/reload 被拒（2）/成功（0）（标准模式）
- `provider-default-model` 供应商级默认模型兜底：模型映射未命中时回退 default_model，同协议仍仅重写 model 字段（标准模式）
