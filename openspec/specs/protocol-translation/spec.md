...
#### Scenario: Codex 工具编排形态
- **WHEN** Codex 以 `additional_tools`（namespace 树内 function/custom 工具编排）携带工具且上游协议非 openai-responses
- **THEN** namespace 展平为点连名 function 定义；custom 型合成 `{code:string}` schema，响应/历史还原 `custom_tool_call`
