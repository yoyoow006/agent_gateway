...
- 核心语义：虚拟令牌→项目档案；同协议字节透传（model_map 时仅重写 model 字节）；跨协议经 IR 翻译；请求边界 failover（408/429/5xx/529/401/403/网络失败换家重放，64MiB 缓冲上限 413）；被动熔断（3 败→60s 指数冷却上限 15m→半开单探针，热重载不复位）；粘性首选。
- 已接受边界（用户 2026-09-03）：Codex ≥0.149 的 additional_tools/namespace/custom 工具编排跨协议不支持（丢弃+告警），Codex 需配 openai-responses 供应商。
